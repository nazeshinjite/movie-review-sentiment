...
| 2026-07-25 | Output naming settled at the folder level: everything in `outputs/` takes its notebook's number, including `predictions/` (`02-lr_val.parquet`, `03-nn_val.parquet`, `04-test_predictions.parquet`). Prompted by a contradiction in `notebooks/README.md` — the convention sentence said one thing and the contract table another, and 02 correctly followed the table. Settled now because 03 and 04 do not exist yet, so the rename costs nothing; once written it would mean touching three notebooks. | Ian (raised on PR #5; confirm with Keana before 03) | — (no artifacts invalidated; 02 renames on its next push) |
